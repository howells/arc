...
`status="blocked"` → `BLOCKED`; any `done` plus any absent/pending → `IN PROGRESS`; all tasks
`status="done"` plus schema-2 `Closeout: passed` → `DONE`; schema-2 all-done with closeout
pending or absent → `IN PROGRESS` with a "closeout required" note. For an unversioned/schema-1
legacy plan, all tasks done → `DONE` through the historical compatibility path. An absent task
status is the legacy spelling of pending. Only `*-implementation.md` files get rows — RFCs and
other documents in the directory are never indexed.

...
while IFS= read -r -d '' f; do
  ...
  # Must have closing ---
  frontmatter_end=$(head -50 "$f" | grep -n "^---$" | sed -n '2p' | cut -d: -f1 || true)
...
while IFS= read -r -d '' f; do
  frontmatter_end=$(head -50 "$f" | grep -n "^---$" | sed -n '2p' | cut -d: -f1 || true)
  [ -z "$frontmatter_end" ] && continue
  name=$(head -"$frontmatter_end" "$f" | grep "^name:" | head -1 | sed 's/name: *//' || true)
  if [ -n "$name" ]; then
...
# ─── 15. Codex mirror freshness ───────────────────────────────────────────────
# The committed plugins/arc is a real-file mirror of the root Arc payload,
# generated by scripts/build-codex-plugin.sh. If a root payload file changes
# without rebuilding, the mirror drifts. Regenerate into a temp dir (fast, and
# ARC_CODEX_OUT keeps the committed mirror + marketplace manifest untouched) and
# diff — a mismatch means someone forgot to rebuild.
if [ -f scripts/build-codex-plugin.sh ] && [ -d plugins/arc ]; then
  echo "Checking Codex mirror freshness..."
  mirror_tmp=$(mktemp -d)
  if ARC_CODEX_OUT="$mirror_tmp/arc" bash scripts/build-codex-plugin.sh > /dev/null 2>&1; then
    if diff -r plugins/arc "$mirror_tmp/arc" > /dev/null 2>&1; then
      ok "Codex mirror (plugins/arc) is in sync"
    else
      error "Codex mirror (plugins/arc) is stale — run scripts/build-codex-plugin.sh"
    fi
  else
    warn "Could not regenerate Codex mirror to verify freshness"
  fi
  rm -rf "$mirror_tmp"
fi

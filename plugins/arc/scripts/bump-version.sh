...
cmd_bump_patch() {
  # Read current version from the first declared file, increment patch, bump all.
  local first_path first_field current
  IFS=$'\t' read -r first_path first_field < <(declared_files | head -1)
  current=$(read_json_field "$REPO_ROOT/$first_path" "$first_field")
  IFS='.' read -r maj min pat <<< "$current"
  local new="$maj.$min.$((pat + 1))"
  cmd_bump "$new"
}
...
case "${1:-}" in
  ...
  --bump-patch)
    ...
    ;;
  --help|-h|"")
    echo "Usage: bump-version.sh <new-version> | --check | --audit | --bump-patch"
    echo ""
    ...
    echo "  --bump-patch   Increment patch on all declared files (used by post-commit)"
    exit 0
    ;;
  --*)

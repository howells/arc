...
color: cyan
description: |
  Use this agent when you have a specification, plan, feature description, or technical
  document that needs user flow analysis and gap identification. Use it to map user
  journeys, identify edge cases, surface missing requirements, and validate completeness
  before implementation begins on complex user-facing features.
model: sonnet
website:
  desc: User flow gap finder
  summary: Analyzes specs for user flows, edge cases, and missing requirements before implementation begins.
  what: |
    The spec flow analyzer examines specifications through the user's lens. It maps every user journey, identifies decision points and branches, considers error states and edge cases, and surfaces gaps with specific, actionable questions. It thinks like a user walking through the feature.
  why: |
